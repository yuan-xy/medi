...
from medi._compatibility import force_unicode
...
def _check_for_exception_catch(node_context, medi_name, exception, payload=None):
    """
    Checks if a medi object (e.g. `Statement`) sits inside a try/catch and
    ...
    def check_try_for_except(obj, exception):
        ...
        for branch_type in iterator:
            ...
            suite = next(iterator)
            if branch_type == 'try' \
                    and not (branch_type.start_pos < medi_name.start_pos <= suite.end_pos):
                return False
