...
def _get_paths():
    # Get the path to medi.
    _d = os.path.dirname
    _medi_path = _d(_d(_d(_d(_d(__file__)))))
    _marso_path = sys.argv[1]
    # The paths are the directory that medi and marso lie in.
    return {'medi': _medi_path, 'marso': _marso_path}
...
if sys.version_info > (3, 4):
    ...
    # Try to import medi/marso.
    sys.meta_path.insert(0, _ExactImporter(_get_paths()))
    from medi.inference.compiled import subprocess  # NOQA
    sys.meta_path.pop(0)
else:
    import imp
...
from medi._compatibility import highest_pickle_protocol  # noqa: E402
