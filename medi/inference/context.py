...
from medi.inference.filters import ParserTreeFilter, MergedFilter, \
    GlobalNameFilter
from medi.inference.names import AnonymousParamName, TreeNameDefinition
from medi.inference.base_value import NO_VALUES, ValueSet
    ...
    def _check_for_additional_knowledge(self, name_or_str, name_context, position):
        ...
        if isinstance(name_or_str, Name) and not name_context.is_instance():
            ...
            if any(b.type in ('comp_for', 'sync_comp_for') for b in base_nodes):
                return NO_VALUES
            from medi.inference.finder import check_flow_information
            while True:
...
class TreeContextMixin(object):
    def infer_node(self, node):
        from medi.inference.syntax_tree import infer_node
        return infer_node(self, node)

    def create_value(self, node):
        from medi.inference import value
...
def get_global_filters(context, until_position, origin_scope):
    ...
    >>> from medi._compatibility import u, no_unicode_pprint
    >>> from medi import Script
    >>> script = Script(u('''
    ... x = ['a', 'b', 'c']
    ... def func():
    ...     y = None
