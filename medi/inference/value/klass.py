...
from medi._compatibility import use_metaclass
from medi.parser_utils import get_cached_parent_scope, expr_is_dotted
from medi.inference.cache import inference_state_method_cache, CachedMetaClass, \
    inference_state_method_generator_cache
from medi.inference import compiled
from medi.inference.lazy_value import LazyKnownValues, LazyTreeValue
from medi.inference.filters import ParserTreeFilter
from medi.inference.names import TreeNameDefinition, ValueName
from medi.inference.arguments import unpack_arglist, ValuesArguments
from medi.inference.base_value import ValueSet, iterator_to_value_set, \
    NO_VALUES
from medi.inference.context import ClassContext
from medi.inference.value.function import FunctionAndClassBase
from medi.inference.gradual.generics import LazyGenericManager, TupleGenericManager
...
class ClassName(TreeNameDefinition):
    ...
    def infer(self):
        ...
        from medi.inference.syntax_tree import tree_name_to_values
...
class ClassMixin(object):
    ...
    def get_filters(self, origin_scope=None, is_instance=False, include_metaclasses=True):
        ...
        if not is_instance:
            from medi.inference.compiled import builtin_from_name
    ...
    def is_typeddict(self):
        # TODO Do a proper mro resolution. Currently we are just listing
        # classes. However, it's a complicated algorithm.
        from medi.inference.gradual.typing import TypedDictClass
        for lazy_cls in self.py__bases__():
...
class ClassValue(use_metaclass(CachedMetaClass, ClassMixin, FunctionAndClassBase)):
    ...
    def list_type_vars(self):
        ...
        for stars, node in unpack_arglist(arglist):
            ...
            from medi.inference.gradual.annotation import find_unknown_type_vars
