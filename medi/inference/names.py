...
from medi.parser_utils import find_statement_documentation, clean_scope_docstring
from medi.inference.utils import unite
...
from medi.inference.helpers import deep_ast_copy, infer_call_of_leaf
...
class ValueNameMixin(object):
    ...
    def py__doc__(self):
        ...
        if not doc and self._value.is_stub():
            from medi.inference.gradual.conversion import convert_names
...
class _ActualTreeParamName(BaseTreeParamName):
    ...
    def infer_annotation(self, execute_annotation=True, ignore_stars=False):
        from medi.inference.gradual.annotation import infer_param
        values = infer_param(
...
class AnonymousParamName(_ActualTreeParamName):
    ...
    def infer(self):
        ...
        if values:
            return values
        from medi.inference.dynamic_params import dynamic_param_lookup
        ...
        if param.star_count == 1:
            from medi.inference.value.iterable import FakeTuple
            value = FakeTuple(self.function_value.inference_state, [])
        elif param.star_count == 2:
            from medi.inference.value.iterable import FakeDict
...
class ParamName(_ActualTreeParamName):
    ...
    def get_executed_param_name(self):
        from medi.inference.param import get_executed_param_names
